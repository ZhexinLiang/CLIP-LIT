            ...
            for iteration, item in enumerate(train_loader): 
                ...
                with torch.no_grad():
                    ...
                    if (sum(score_psnr).item()/30.0)>max_score_psnr and ((total_iteration+1) % config.display_iter) == 0:
                        ...
                        inference(config.lowlight_images_path,'./'+task_name+'/result_'+task_name+'/result_jt_'+str(total_iteration+1)+"_psnr"+str(max_score_psnr)+'/',U_net,256)
                        if total_iteration >config.num_reconstruction_iters+config.num_clip_pretrained_iters:
                            semi_path[pr_semi_path]='./'+task_name+'/result_'+task_name+'/result_jt_'+str(total_iteration+1)+"_psnr"+str(max_score_psnr)+'/'
                            print(semi_path)
